...
<?PHP
	...
	if(isset($_POST["submit"])){
		...
		$sql = "SELECT * FROM `Users` where `FName` = '$Fname'"; 
			...
			//$oldOne = $row["Balance"];	
			$sql="INSERT INTO `ATM`.`Trans` (`ID`, `Name`, `Type`, `Amount`, `Date`) VALUES ('$ID', '$Name', 'Deposit', '$Balance', '');
";			
			$query=$db->query($sql);
		...
		$sql="UPDATE `ATM`.`Users` SET `Balance` = '$newOne' WHERE `users`.`FName` = '$Fname' ;";
		 ...
		 echo "Successfully";
			 ...
			 echo "There is problem while Deposit please check again ;)";
...
<form action="" method="POST">
		...
		<td colspan="2">
		<input type="submit" class="btn" name="submit" value="Deposit"></td>
			</table>
